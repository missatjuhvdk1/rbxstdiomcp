...
### v1.7.x
- **Partial Script Editing:** `edit_script` (string-based, with post-edit syntax validation) for targeted edits without rewriting entire scripts. *(The legacy line-based variants `edit_script_lines`, `insert_script_lines`, `delete_script_lines` were removed in favor of `edit_script` — line numbers shift after every edit and made the legacy tools unreliable.)*
